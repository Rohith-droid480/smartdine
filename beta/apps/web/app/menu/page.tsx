  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (tableRes.success && tableRes.data) {
        ...
        const occupiedOrFree = tableRes.data.find(t => t.status === 'occupied') || tableRes.data[0];
        if (occupiedOrFree) setSelectedTableId(occupiedOrFree.id);
      }
    } catch (err) {
                ...
                >
                  {tables.map((t) => (
                    <option key={t.id} value={t.id}>
                      Table {t.number} ({t.location?.replace('_', ' ') || 'Main'}) — {t.capacity} Seats
                    </option>
                  ))}
                </select>
              </div>
