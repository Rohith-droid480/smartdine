...
import { ALLOWED_STOCK_STATUSES, getStockStatusLabel } from '@/lib/inventory-utils';
...
export const InventoryFilters: React.FC<InventoryFiltersProps> = ({
  ...
  const filterOptions = ['all', ...ALLOWED_STOCK_STATUSES];

  return (
    <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
      {filterOptions.map((filter) => {
        const isSelected = currentFilter === filter;
        const label = filter === 'all' ? 'All Ingredients' : getStockStatusLabel(filter);
