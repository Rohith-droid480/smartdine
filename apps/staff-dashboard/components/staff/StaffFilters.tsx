...
import { ALLOWED_SHIFT_STATUSES, getShiftStatusLabel } from '@/lib/staff-utils';
...
export const StaffFilters: React.FC<StaffFiltersProps> = ({
  ...
  const filterOptions = ['all', ...ALLOWED_SHIFT_STATUSES];

  return (
    <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
      {filterOptions.map((filter) => {
        const isSelected = currentFilter === filter;
        const label = filter === 'all' ? 'All Staff' : getShiftStatusLabel(filter);
