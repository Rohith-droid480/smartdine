    ...
    try {
      if (token) {
        ...
        if (res.success && res.data) {
          const found = orderId ? res.data.find((o: Order) => o.id === orderId) : res.data[0];
          setOrder(found || res.data[0] || null);
        }
      }
    } catch (e) {
      console.error('Failed to fetch billing order:', e);
    } finally {
      setLoading(false);
    }
  };
        ...
        <Card className="glass-panel p-8 sm:p-10 space-y-8 border border-slate-700/80 shadow-2xl relative">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs bg-slate-900/60 p-4 rounded-xl border border-slate-800">
            ...
            <div>
              ...
              <span className="font-medium text-slate-200">Table {(order as any).tableNumber ?? order.tableId?.slice(0, 4) ?? 'Dine-In'}</span>
            </div>
            <div>
              ...
              {order.items.map((item: OrderItem, idx: number) => (
                <div key={idx} className="py-3 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-slate-200">{(item as any).name || 'Dish Item'}</span>
                  </div>
